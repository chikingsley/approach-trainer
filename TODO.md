...
- [ ] **Verify factory backlog finished** — 1,528 new videos (Douyin Mandarin + nstv/sergio/shorts English) running through `factory.py` (batched 5× scribe → consensus → segment). Confirm new `clips` rows + `segments` written; re-run `factory.py` to mop up any stragglers (downloads still arriving).
...
- [ ] **Diarized, consensus SRT export** — generate `.srt` (diarized, from our turns w/ start-end) for courses + clips. User expected SRTs to exist for the courses; they don't. Tie this into the non-mp4 ingest below (those especially need SRTs).
...
- [ ] **OCR consensus** — *now* worth building: a multi-model OCR consensus (5.3-chat + 5.2 + 5.4-mini agree) mirroring text `consensus_fuse`. Benchmark shows the top 3 are close + complementary.
...
- [ ] **OCR consensus** — build a multi-model OCR consensus pass (GPT-family only for vision) mirroring the text `consensus_fuse`.
...
- [ ] **Meta: refactor the API** — we've repeated consensus and OCR motions across projects; consider consolidating these patterns into the package.
...
- [x] **782/782 long-form `sources` transcribed** — diarized 5× Scribe → consensus → SQLite (courses: Project GO, Tyler, Tate, Todd, Hot Seat; + RSD Julien YouTube archive & Transformation Mastery & Pimp).
...
### Factory pipeline
- [x] **`factory.py` built** — per-item + batched: scan media roots → detect cuts → extract audio → 5× scribe (per-language, `--max-workers 200`) → consensus → segment → write rows. Resumable/idempotent.
