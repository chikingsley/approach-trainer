...
set -u
if [ "$#" -lt 2 ]; then
  echo "Usage: reprocess_ids.sh <creator-dir-name> <id> [<id> ...]" >&2
  exit 2
fi

exec 9>/tmp/reprocess.lock
...
for id in "$@"; do
    ...
    ffmpeg -nostdin -y -v error -i "$src" -vn -ac 1 -ar 16000 -c:a flac "$WORK/$id.flac" 2>/dev/null
echo "$WORK/$id.flac" >> "$WORK/paths.txt"
done
echo "audio: $(wc -l < "$WORK/paths.txt") clips"

for r in 0 1 2 3 4; do
    uv run --project "$APPROACH" superwhisper-audio --paths-file "$WORK/paths.txt" --jsonl "$WORK/runs/run$r.jsonl" \
      --diarize --language eng --max-workers 4 >/dev/null 2>&1
    echo "run $r: $(wc -l < "$WORK/runs/run$r.jsonl" 2>/dev/null) results"
done
uv run --project "$APPROACH" "$APPROACH/scripts/process_full.py" "$WORK/runs" "$DB"
echo "DONE" > "$WORK/done"
