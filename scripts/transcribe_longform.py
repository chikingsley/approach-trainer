...
Resumable via the DB. Run in tajik env:  uv run python transcribe_longform.py
"""
import hashlib
import json
import subprocess
...
TAJIK = Path.home() / "github/peacock-asr/projects/tajik-asr"
RUNS = 5
WORKERS = 48
...
EN_INSTRUCTION = (
    ...
    "and filler. No speaker labels or commentary. Output ONLY the transcript in <transcript></transcript>."
)
...
def extract(v: Path):
    ...
    if not flac.exists():
        r = subprocess.run(["ffmpeg", "-y", "-v", "error", "-i", str(v), "-vn", "-ac", "1",
                            "-ar", "16000", "-c:a", "flac", str(flac)], check=False)
        if r.returncode != 0:
            return None
    return str(flac)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=8) as pool:
        for v, flac in zip(vids, pool.map(extract, vids)):
            if flac:
                fid2v[fid_of(v)] = v
    (AUD / "paths.txt").write_text("\n".join(str(AUD / f"{f}.flac") for f in fid2v))
    print(f"extracted {len(fid2v)} audio files; running {RUNS}x diarized Scribe...", flush=True)

    # 5 diarized Scribe passes (proven batched entry, 200 workers each)
    for r in range(RUNS):
        subprocess.run(["uv", "run", "superwhisper-audio", "--paths-file", str(AUD / "paths.txt"),
                        "--jsonl", str(AUD / f"run{r}.jsonl"), "--diarize", "--language", "eng",
                        "--max-workers", "200"], cwd=TAJIK, check=False)
    ...
    def process(fid):
        ...
        best = max(recs, key=lambda r: len(r.get("raw_response", {}).get("words", []))) if recs else {}
        ...
        return fid, (fid, str(v), collection, v.stem, kind, "en",
                     round(float(best.get("duration") or 0), 2), len(speakers),
                     resolved, json.dumps(turns, ensure_ascii=False),
                     json.dumps(speakers), "resolved"), \
            [(fid, i, r.get("transcript", "")) for i, r in enumerate(recs)]

    n = 0
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        for fid, row, runs in pool.map(process, list(fid2v)):
            ...
            db.executemany("INSERT OR REPLACE INTO runs (clip_id,run_idx,text,created_at) VALUES (?,?,?,datetime('now'))", runs)
            db.commit()
            n += 1
            if n % 50 == 0:
