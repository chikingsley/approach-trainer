...
scribe_lang() {
  ...
  for r in 0 1 2 3 4; do
    ...
    uv run superwhisper-audio --paths-file "$paths" --jsonl "$outdir/run$r.jsonl" \
...
uv run python "$HOME/github/approach-trainer/scripts/process_full.py" /tmp/full/en "$DB" >> /tmp/full/pipeline.log 2>&1
uv run python "$HOME/github/approach-trainer/scripts/process_full.py" /tmp/full/ru "$DB" >> /tmp/full/pipeline.log 2>&1
